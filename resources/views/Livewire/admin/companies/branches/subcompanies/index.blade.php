                ...
                <table class="w-full table-fixed text-sm">
                    <thead>
                        <tr class="text-left text-xs font-black tracking-wide uppercase text-gray-600 border-b border-gray-200">
                            <th class="py-3 pr-4 w-[20%]">Sub Company</th>
                            <th class="py-3 pr-4 w-[10%]">Code</th>
                            <th class="py-3 pr-4 w-[17%]">Email</th>
                            <th class="py-3 pr-4 w-[12%]">Phone</th>
                            <th class="py-3 pr-4 w-[14%]">Location</th>
                            <th class="py-3 pr-4 w-[8%]">Status</th>
                            <th class="py-3 pr-0 w-[19%] text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($subCompanies as $sub)
                            <tr class="{{ $sub->is_active ? '' : 'opacity-60 grayscale' }}">
                                <td class="py-4 pr-4 align-top">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full border border-gray-200 bg-white flex items-center justify-center overflow-hidden flex-shrink-0">
                                            @if ($sub->logo_path)
                                                <img src="{{ '/storage/' . ltrim($sub->logo_path, '/') }}" alt="Logo" class="w-full h-full object-contain p-1">
                                            @else
                                                <span class="text-xs font-black text-gray-500">
                                                    {{ strtoupper(mb_substr((string) $sub->name, 0, 2)) }}
                                                </span>
                                            @endif
                                        </div>
                                        <div class="min-w-0">
                                            <div class="font-black text-gray-900 truncate">{{ $sub->name }}</div>
                                            <div class="text-xs text-gray-500 mt-0.5">{{ optional($sub->created_at)->format('Y-m-d') }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-4 pr-4 align-top font-semibold text-gray-900 break-words">{{ $sub->code }}</td>
                                <td class="py-4 pr-4 align-top font-semibold text-gray-900 break-all">{{ $sub->email }}</td>
                                <td class="py-4 pr-4 align-top font-semibold text-gray-900 break-words">{{ $sub->phone ?: '—' }}</td>
                                <td class="py-4 pr-4 align-top font-semibold text-gray-900 break-words">
                                    {{ $sub->city }}, {{ $sub->country }}
                                </td>
                                <td class="py-4 pr-4 whitespace-nowrap">
                                    <span class="inline-flex items-center rounded-md {{ $sub->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-700' }} px-2 py-0.5 text-[11px] font-black">
            ...
            <div class="relative h-full w-full overflow-y-auto px-4 py-8">
                <div class="mx-auto w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-xl border border-gray-200">
                    <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-white to-[#f2feff]">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <div class="text-lg font-black text-gray-900">Add Sub Company</div>
                                <div class="text-xs text-gray-600 mt-0.5">{{ $company->name }} · {{ $branch->name }}</div>
                            </div>
