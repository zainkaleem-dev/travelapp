    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div class="px-6 py-3.5 bg-gradient-to-r from-white to-[#f2feff] border-b border-gray-200">
            <div class="flex items-start justify-between gap-4">
                <h1 class="text-[21px] font-black text-gray-900 tracking-tight">Add Country</h1>
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            @error('country_code') <p class="mt-1 text-[11px] font-medium text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-gray-500">Country Name</label>
                            <input type="text" wire:model.defer="country_name" placeholder="United States"
                                class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-[11px] text-gray-800 shadow-sm focus:border-[#2ab4c0] focus:outline-none focus:ring-2 focus:ring-[#2ab4c0]/25">
                            @error('country_name') <p class="mt-1 text-[11px] font-medium text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-gray-500">Dial Code (e.g. +1)</label>
                            <input type="text" wire:model.defer="country_dial_code" placeholder="+1"
                                class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-[11px] text-gray-800 shadow-sm focus:border-[#2ab4c0] focus:outline-none focus:ring-2 focus:ring-[#2ab4c0]/25">
                            @error('country_dial_code') <p class="mt-1 text-[11px] font-medium text-red-600">{{ $message }}</p> @enderror
                        </div>
                    </div>
