<div>
    <div class="bg-indigo-600 text-white py-1.5 text-center text-xs relative">
        ...
        <div class="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 items-center gap-4 text-white/80">
            <button class="flex items-center gap-1 hover:text-white transition-colors">App</button>
            <button class="flex items-center gap-1 hover:text-white transition-colors">Support</button>
            <button class="flex items-center gap-1 hover:text-white transition-colors">English</button>
        </div>
    </div>
        ...
        <div class="w-full max-w-sm">
            <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                ...
                <div class="px-6 py-6 space-y-4">
                    ...
                    @if($successMessage)
                        <div class="fu2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-green-700" style="font-size:11px">
                            {{ $successMessage }}
                        </div>
                    @endif

                    @if($errorMessage)
                        <div class="fu2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-red-700" style="font-size:11px">
                            {{ $errorMessage }}
                        </div>
                    @endif
                    ...
                    <form wire:submit="sendResetLink" novalidate>
                        ...
                        <button type="submit" class="fu6 login-btn w-full py-2.5 rounded-lg text-white font-bold flex items-center justify-center gap-2 mt-4" style="font-size:13px">
                            Send Reset Link
                        </button>
                    </form>
