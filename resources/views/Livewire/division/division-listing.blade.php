                    ...
                    <table class="w-full border-separate border-spacing-0">
                        ...
                        <tbody>
                            @forelse ($divisions as $division)
                                <tr class="border-b border-gray-200 transition-colors {{ $division->status === 'active' ? 'hover:bg-blue-50' : 'bg-gray-100' }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <button wire:click="toggleActive({{ $division->id }})" class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold capitalize transition-all duration-200
                                            {{ $division->status === 'active' ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200' }}">
                                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $division->status === 'active' ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                            {{ $division->status }}
                                        </button>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <button type="button" x-on:click="appSwalFromDataset($el, $wire)"
                                                data-action="delete"
                                                data-args='[{{ $division->id }}]'
                                                data-confirm-title="Delete division?"
                                                data-done-title="Division deleted"
                                                class="group inline-flex items-center justify-center p-1 rounded-lg border border-gray-200 bg-transparent text-black text-[11px] font-semibold transition-colors hover:border-gray-400"
                                                title="Delete">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-50">
                        <div class="flex items-center justify-between">
                            <div class="text-[11px] text-gray-500 font-medium">
                                ...
                                <span class="font-bold text-gray-900">{{ $paginationMeta['total'] }}</span> divisions
                            </div>
