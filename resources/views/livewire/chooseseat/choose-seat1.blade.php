        ...
        <div class="bg-[#2ab4c0] text-white rounded-xl px-4 py-3 mb-6 flex items-center justify-between flex-wrap gap-2 shadow-lg shadow-[#2ab4c0]/20"
            ...
            <div class="text-right">
                ...
                <p class="text-base font-bold text-white">
                    {{ \Carbon\Carbon::parse($searchParams['departDate'] ?? now())->format('l d.m.Y') }}</p>
            </div>
        </div>
    </div>
                                ...
                                @foreach($segments as $sIdx => $segment)
                                    ...
                                    <div wire:click="changeSegment({{ $segIdx }})" wire:key="segment-{{ $segIdx }}"
                                        ...
                                        <div
                                            ...
                                            <div
                                                class="w-12 h-12 flex items-center justify-center rounded-lg bg-white border border-gray-100 p-1 shadow-sm">
                                                <img src="https://pics.avs.io/128/128/{{ $carrierCode }}.png"
                                                    alt="{{ $airlineName }}" class="w-full h-full object-contain">
                                            </div>
                                            <div class="flex flex-col">
                                                <span
                                                    class="text-[10px] font-bold text-gray-500 uppercase tracking-tighter leading-none">{{ $airlineName }}</span>
                                                @if($currentSegmentIndex === $segIdx)
                                                    <span
                                                        class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-[#2ab4c0] text-[8px] font-black text-white uppercase tracking-widest mt-1 w-max">
                                        ...
                                        <div class="flex flex-col justify-center gap-1.5">
                                            ...
                                            <div class="flex justify-center">
                                                <span
                                                    class="text-[10px] text-gray-500 tracking-[0.22em] uppercase">{{ $flightNumber }}</span>
                                            </div>

                                            {{-- Times --}}
                                            <div
                                                class="flex items-center gap-3 text-xl sm:text-2xl font-black text-gray-900 tracking-tighter">
                                                <div class="flex flex-col">
                                                    <span
                                                        class="text-[10px] font-bold text-gray-400 tracking-[0.18em] uppercase">Depart</span>
                                                    <span>{{ $depTime }}</span>
                                                </div>
                                                <div class="flex items-center justify-center flex-1 mx-1">
                                                    <div class="w-full max-w-[140px] h-[2px] bg-gray-200 rounded-full relative">
                                                        <div
                                                            class="absolute inset-y-[-3px] left-1/2 -translate-x-1/2 flex items-center justify-center">
                                                            <span
                                                                class="px-1.5 py-0.5 rounded-full bg-white border border-gray-200 text-[9px] font-black text-gray-500 tracking-[0.18em] uppercase">
                                                                Non‑stop
                                                            </span>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="flex flex-col items-end">
                                                    <span
                                                        class="text-[10px] font-bold text-gray-400 tracking-[0.18em] uppercase">Arrive</span>
                                                    <span>{{ $arrTime }}</span>
                                                </div>
                                            </div>
                                            ...
                                            {{-- Route trail --}}
                                            <div
                                                class="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.22em] text-gray-500">
                                            ...
                                            @if(!empty($segmentChosenSeats))
                                                <div class="flex flex-wrap gap-1 justify-center mt-1">
                                                    @foreach($segmentChosenSeats as $seatStr)
                                                        <span
                                                            class="px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 text-[9px] font-black border border-indigo-100 shadow-sm">{{ $seatStr }}</span>
                                                    @endforeach
                                                </div>
                                            @else
                                                <div class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-0.5">
                                                    Non-stop</div>
                                            @endif
                                        </div>
                ...
                @if(isset($flightInfo[$currentSegmentIndex]))
                    ...
                    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                        <div>
                            ...
                            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-1">Flight
                                {{ $currSeg['flightNumber'] }} • Segment {{ $currentSegmentIndex + 1 }} of
                                {{ count($flightInfo) }}</p>
                        </div>
                        <div class="hidden md:flex gap-4">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="w-3.5 h-3.5 rounded-sm bg-sky-100 border border-sky-200 relative">
                                        <div class="absolute -top-1 left-1.5 w-1 h-2 bg-sky-400 rounded-full"></div>
                                    </div>
                            ...
                            <div class="flex flex-col gap-1.5">
                                <div class="flex items-center gap-2">
                                    <div
                                        class="w-3.5 h-3.5 rounded-sm bg-gray-50 border border-gray-100 text-gray-300 flex items-center justify-center text-[10px] font-bold">
                                        ×</div>
                                    <span class="text-[10px] font-bold text-gray-400 uppercase">Occupied</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <div
                                        class="w-3.5 h-3.5 rounded-sm bg-[#2ab4c0] shadow-sm shadow-[#2ab4c0]/30 text-white flex items-center justify-center text-[10px] font-bold">
                                        14A</div>
                            ...
                            <div class="flex items-center mb-6 pl-8">
                                <div class="flex gap-2 mr-5">
                                    @foreach($leftCols as $col)
                                        <div class="w-[40px] text-center text-gray-400 font-black text-xs uppercase">{{ $col }}
                                        </div>
                                    @endforeach
                                </div>
                                <div class="w-6"></div>
                                ...
                                @foreach($rows as $row)
                                    @if($row['isExtra'] && !$loop->first)
                                        <div class="h-6 flex items-center justify-center mt-6 mb-4">
                                            <div
                                                class="px-4 py-1 bg-sky-50 text-sky-600 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border border-sky-100 flex items-center gap-2 shadow-sm">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                    ...
                                    <div class="flex items-center">
                                        <div
                                            class="w-8 text-center text-gray-400 font-black text-xs flex-shrink-0 relative focus-within:z-50">
                                            {{ $row['number'] }}
                                        </div>
                                            ...
                                            @foreach($leftCols as $col)
                                                @if(isset($row['seats'][$col]))
                                                    ...
                                                    <button wire:click="selectSeat('{{ $seat['id'] }}', {{ $seat['price'] }})"
                                                        ...
                                                        title="{{ $seat['id'] }} - {{ $currencyCode }}{{ number_format($seat['price'], 2) }}">
                                                        <span>{{ $seat['id'] }}</span>
                                                    </button>
                                                @else
            ...
            <div
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="pt-5 mt-2 border-t border-gray-100">
                        ...
                        <p class="text-[9px] text-gray-400 font-medium">Prices include all taxes and carrier-imposed
                            fees.</p>
                    </div>
