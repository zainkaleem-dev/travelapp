<div class="w-full px-1 py-1">
    <div class="rounded-lg border border-gray-200 bg-white shadow-sm mb-4">
        <div class="px-6 py-3.5 bg-gradient-to-r from-white to-[#f2feff] border-b border-gray-200">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h1 class="text-[21px] font-black text-gray-900 tracking-tight">Edit Department</h1>
                    <p class="text-[11px] text-gray-500 mt-1">Modify the existing department</p>
                </div>
            ...
            <div class="space-y-8">
                ...
                <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-[11px] font-black tracking-widest text-gray-400 uppercase">Department Information</h2>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <div class="relative"
                                x-data="{ open: false, selected: @entangle('division_id').live, labels: @js(collect($divisions)->pluck('name', 'id')) }"
                                @keydown.escape.window="open = false" @click.outside="open = false">
                                <button type="button" class="input-field flex items-center justify-between text-left" @click="open = !open">
                                    <span x-text="!selected ? '-- Choose Division --' : (labels[selected] ?? '-- Choose Division --')"></span>
                                ...
                                <div x-cloak x-show="open" x-transition.origin.top class="admin-menu-panel">
                                    <button type="button" class="admin-menu-item" :class="{ 'is-active': !selected }" @click="selected = ''; open = false">-- Choose Division --</button>
                                    @foreach($divisions as $division)
                                        <button type="button" class="admin-menu-item" :class="{ 'is-active': String(selected) === '{{ $division->id }}' }" @click="selected = '{{ $division->id }}'; open = false">{{ $division->name }}</button>
                                    @endforeach
                                </div>
                            </div>
                            @error('division_id') <p class="mt-1 text-[11px] font-bold text-red-500 uppercase">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="field-label">Department Name <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="name" class="input-field" placeholder="e.g. IT Department">
                    ...
                    <div class="grid grid-cols-1 gap-6 mb-6">
                        <div>
                            ...
                            <textarea wire:model="description" class="input-field min-h-[100px] py-3" placeholder="Brief description of this department..."></textarea>
