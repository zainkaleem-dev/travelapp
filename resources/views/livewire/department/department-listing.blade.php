        ...
        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm mb-4">
            <div class="px-6 py-3.5 bg-gradient-to-r from-white to-[#f2feff] border-b border-gray-200">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h1 class="text-[21px] font-black text-gray-900 tracking-tight">Departments</h1>
                    </div>
                    <div class="flex items-center gap-3">
                        <a href="{{ route('departments.create', ['companyId' => $companyId]) }}"
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-white to-[#f2feff] border-b border-gray-200">
                <div class="w-full">
                    <div class="relative">
                    ...
                    <table class="w-full border-separate border-spacing-0">
                            ...
                            <tr class="border-b-2 border-gray-200 bg-[#2ab4c0]">
                                ...
                                <th class="px-6 py-2 text-start text-[11px] font-bold text-white uppercase tracking-wide cursor-pointer group"
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div class="flex flex-col transition-opacity {{ $sortBy === 'description' ? 'opacity-100' : 'opacity-70' }}">
                                            <svg class="w-3.5 h-3.5 {{ $sortBy === 'description' && $sortDirection === 'asc' ? 'text-white' : 'text-white/70' }}" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" clip-rule="evenodd" />
                                            </svg>
                                            <svg class="w-3.5 h-3.5 -mt-1 {{ $sortBy === 'description' && $sortDirection === 'desc' ? 'text-white' : 'text-white/70' }}" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                            </svg>
                                        </div>
                                    </div>
                                ...
                                <th class="px-6 py-2 text-start text-[11px] font-bold text-white uppercase tracking-wide cursor-pointer group"
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span>Status</span>
                                        <div class="flex flex-col transition-opacity {{ $sortBy === 'status' ? 'opacity-100' : 'opacity-70' }}">
                                            <svg class="w-3.5 h-3.5 {{ $sortBy === 'status' && $sortDirection === 'asc' ? 'text-white' : 'text-white/70' }}" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" clip-rule="evenodd" />
                                            </svg>
                                            <svg class="w-3.5 h-3.5 -mt-1 {{ $sortBy === 'status' && $sortDirection === 'desc' ? 'text-white' : 'text-white/70' }}" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                            </svg>
                                        </div>
                                    </div>
                                </th>
                                <th class="px-6 py-2 text-start text-[11px] font-bold text-white uppercase tracking-wide rounded-se-lg">
                                    Actions
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($departments as $department)
                                <tr class="border-b border-gray-200 transition-colors {{ $department->status === 'active' ? 'hover:bg-blue-50' : 'bg-gray-100' }}">
                                    <td class="px-6 py-4 text-[11px] font-semibold text-gray-900">
                                        {{ $department->name }}
                                    </td>
                                    <td class="px-6 py-4 text-[11px] text-gray-700">
                                        {{ $department->division->name ?? 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 text-[11px] text-gray-700">
                                        {{ $department->description }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <button wire:click="toggleActive({{ $department->id }})" class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold capitalize transition-all duration-200
                                            {{ $department->status === 'active' ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200' }}">
                                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $department->status === 'active' ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                            {{ $department->status }}
                                        </button>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-1">
                                            <a href="{{ route('departments.edit', ['companyId' => $companyId, 'id' => $department->id]) }}"
                                            ...
                                            <button type="button" x-on:click="appSwalFromDataset($el, $wire)"
                                                data-action="delete"
                                                data-args='[{{ $department->id }}]'
                                                data-confirm-title="Delete department?"
                                                data-done-title="Department deleted"
                                                class="group inline-flex items-center justify-center p-1 rounded-lg border border-gray-200 bg-transparent text-black text-[11px] font-semibold transition-colors hover:border-gray-400"
                                                title="Delete">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-50">
                        <div class="flex items-center justify-between">
                            <div class="text-[11px] text-gray-500 font-medium">
                                ...
                                <span class="font-bold text-gray-900">{{ $paginationMeta['total'] }}</span> departments
                            </div>
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    @for ($page = max(1, $paginationMeta['current_page'] - 2); $page <= min($paginationMeta['last_page'], $paginationMeta['current_page'] + 2); $page++)
                                        <button wire:click="goToPage({{ $page }})"
                                            ...
                                            {{ $page === $paginationMeta['current_page'] ? 'bg-[#2ab4c0] text-white shadow-md' : 'border border-gray-100 text-gray-500 hover:bg-gray-50' }}">
                                            {{ $page }}
                                        </button>
                                    @endfor
