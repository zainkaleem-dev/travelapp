<div class="w-full">
    <div class="px-1 py-1 w-full">
        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm mb-4">
            <div class="px-6 py-3.5 bg-gradient-to-r from-white to-[#f2feff] border-b border-gray-200">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h1 class="text-[21px] font-black text-gray-900 tracking-tight">Audit Log Details</h1>
                        <p class="text-[11px] font-bold text-gray-400 uppercase mt-1 tracking-wider">Detailed Activity Report</p>
                    </div>
                    <a href="{{ route('admin.audit-logs') }}"
                        class="inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-[11px] font-bold text-gray-700 hover:bg-gray-50 uppercase tracking-wider transition-colors shadow-sm">
                        Back to List
                    </a>
                </div>
            </div>

            <div class="p-6 space-y-6">
                <div class="rounded-lg border border-gray-200 bg-white p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400">User</p>
                        <p class="mt-1 text-[11px] font-semibold text-gray-900">{{ $this->actorLabel() }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400">Organization</p>
                        <p class="mt-1 text-[11px] font-semibold text-gray-900">{{ $this->organizationLabel() }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400">Branch</p>
                        <p class="mt-1 text-[11px] font-semibold text-gray-900">{{ $this->branchLabel() }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400">Timestamp</p>
                        <p class="mt-1 text-[11px] font-semibold text-gray-900">{{ $activityLog->created_at?->format('d/m/Y (H:i:s)') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400">Action</p>
                        <p class="mt-1 text-[11px] font-semibold text-gray-900">{{ ucwords($activityLog->action_name ?: '-') }}</p>
                    </div>
                </div>

                <div class="rounded-lg border border-gray-200 bg-white p-4">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400 mb-2">Action Page</p>
                    <p class="text-[11px] text-gray-900 font-medium">
                        {{ $this->pageName() }}
                    </p>
                </div>

                <div class="rounded-lg border border-gray-200 bg-white p-4">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400 mb-2">Action Detail</p>
                    <p class="text-[11px] text-gray-800 font-medium leading-relaxed">{{ $this->detailedMessage() }}</p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="rounded-lg border border-gray-200 bg-white p-4">
                        <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400 mb-4 pb-2 border-b border-gray-50">Before State</p>
                        <div class="space-y-3">
                            @forelse($this->beforeState() as $label => $value)
                                <div class="flex justify-between items-start gap-4">
                                    <span class="text-[11px] font-semibold text-gray-400 uppercase tracking-tight">{{ $label }}</span>
                                    <span class="text-[11px] text-gray-800 font-medium text-right">{{ $value }}</span>
                                </div>
                            @empty
                                <p class="text-xs text-gray-400 italic">No previous data captured.</p>
                            @endforelse
                        </div>
                    </div>

                    <div class="rounded-lg border border-gray-200 bg-white p-4">
                        <p class="text-[11px] font-bold uppercase tracking-wider text-gray-400 mb-4 pb-2 border-b border-gray-50">After State</p>
                        <div class="space-y-3">
                            @forelse($this->afterState() as $label => $value)
                                <div class="flex justify-between items-start gap-4">
                                    <span class="text-[11px] font-semibold text-gray-400 uppercase tracking-tight">{{ $label }}</span>
                                    <span class="text-[11px] text-[#2ab4c0] font-bold text-right">{{ $value }}</span>
                                </div>
                            @empty
                                <p class="text-xs text-gray-400 italic">No new data captured.</p>
                            @endforelse
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

