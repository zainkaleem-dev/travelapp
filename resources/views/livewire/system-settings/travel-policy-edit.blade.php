    ...
    <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm mb-4">
        ...
        <div class="px-6 py-3.5 bg-gradient-to-r from-white to-[#f2feff] border-b border-gray-200">
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="text-[11px] text-gray-500 mt-1">Modify the travel policy settings</p>
                </div>
                <a href="{{ $returnUrl ?? route('admin.system-settings', ['activeTab' => 'travel-policy']) }}"
                    class="inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white px-4 py-2 text-[11px] font-semibold text-gray-700 hover:bg-gray-50 uppercase tracking-wider transition-colors">
                    Back
                </a>
            </div>
        </div>
        ...
        <form wire:submit.prevent="save" class="p-6">
            <div class="space-y-6">
                ...
                <div x-show="activeTab === 'general'" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-[11px] font-black tracking-widest text-gray-400 uppercase">Policy
                                Configuration</h2>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" wire:model="name" class="input-field"
                                    placeholder="e.g. Executive Flight Policy">
                                @error('name') <p class="mt-1 text-[11px] font-bold text-red-500 uppercase">
                                {{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative"
                                    ...
                                    <button type="button"
                                        ...
                                        @click="open = !open">
                                        <span
                                            x-text="!selected ? 'Select company...' : (companies[selected] ?? 'Select company...')"></span>
                                    ...
                                    <div x-cloak x-show="open" x-transition.origin.top
                                        class="admin-menu-panel max-h-64 overflow-y-auto">
                                        <button type="button" class="admin-menu-item"
                                            :class="{ 'is-active': !selected }"
                                            @click="selected = ''; open = false">Select company...</button>
                                ...
                                @error('companyId') <p class="mt-1 text-[11px] font-bold text-red-500 uppercase">
                                {{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label class="field-label">Policy Type <span class="text-red-500">*</span></label>
                                <div class="relative" x-data="{ open: false, selected: @entangle('policyType').live }"
                                    @keydown.escape.window="open = false" @click.outside="open = false">
                                    <button type="button"
                                        class="input-field flex items-center justify-between text-left uppercase"
                                        @click="open = !open">
                                    ...
                                    <div x-cloak x-show="open" x-transition.origin.top class="admin-menu-panel">
                                        @foreach($policyTypes as $type)
                                            <button type="button" class="admin-menu-item uppercase"
                                                :class="{ 'is-active': selected === '{{ $type }}' }"
                                                @click="selected = '{{ $type }}'; open = false">{{ $type }}</button>
                                        @endforeach
                                    </div>
                                </div>
                                @error('policyType') <p class="mt-1 text-[11px] font-bold text-red-500 uppercase">
                                {{ $message }}</p> @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea wire:model="description" rows="3" class="input-field pt-2"
                                    placeholder="Policy details and rules..."></textarea>
                            ...
                            <div class="w-full sm:w-1/3">
                                ...
                                <div class="relative" x-data="{ open: false, selected: @entangle('isActive').live }"
                                    @keydown.escape.window="open = false" @click.outside="open = false">
                                    <button type="button"
                                        class="input-field flex items-center justify-between text-left capitalize"
                                        @click="open = !open">
                                    ...
                                    <div x-cloak x-show="open" x-transition.origin.top class="admin-menu-panel">
                                        <button type="button" class="admin-menu-item" :class="{ 'is-active': selected }"
                                            @click="selected = true; open = false">Active</button>
                                        <button type="button" class="admin-menu-item"
                                            :class="{ 'is-active': !selected }"
                                            @click="selected = false; open = false">Inactive</button>
                                    </div>
                                </div>
                                @error('isActive') <p class="mt-1 text-[11px] font-bold text-red-500 uppercase">
                                {{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="activeTab === 'grades'" x-cloak x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 transform -translate-y-2"
                    x-transition:enter-end="opacity-100 transform translate-y-0">
                    <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-[11px] font-black tracking-widest text-gray-400 uppercase">Applicable Grades
                            </h2>
                            <span
                                class="text-[10px] font-bold text-[#2ab4c0] bg-[#2ab4c0]/5 px-2 py-0.5 rounded uppercase">Selection
                                Required</span>
                        </div>
                        ...
                        @if(count($grades) === 0)
                            <div class="text-center py-8 rounded-lg border-2 border-dashed border-gray-100 bg-white">
                                <p class="text-[11px] font-bold text-gray-400 uppercase">No grades found for this
                                    organization.</p>
                                <a href="{{ $companyId ? route('grades.index', ['companyId' => $companyId]) : '#' }}"
                                    class="text-[10px] font-bold text-[#2ab4c0] hover:underline mt-2 inline-block uppercase">Manage
                                    Grades First</a>
                            </div>
                        @endif
                        @error('selectedGrades') <p class="mt-1 text-[11px] font-bold text-red-500 uppercase">
                        {{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-10 pt-6 border-t border-gray-100">
                ...
                <button type="submit"
                    ...
                    <span wire:loading.remove>Update Policy</span>
