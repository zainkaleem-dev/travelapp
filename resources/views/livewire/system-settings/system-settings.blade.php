        ...
        <div class="px-6 pt-2 border-b border-gray-200 bg-white">
            <div class="flex items-center gap-0 overflow-x-auto no-scrollbar text-[11px] font-semibold w-full">
                <button wire:click="$set('activeTab', 'endpoints')"
                    class="inline-flex items-center gap-1.5 px-4 py-2 flex-shrink-0 rounded-t-lg bg-[#2ab4c0] text-white font-semibold">
        ...
        <div x-show="filtersOpen" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex flex-col sm:flex-row items-end gap-4 animate-fade-in">
                <div class="w-full sm:w-64">
                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 block">Select
                        Company</label>
                    <div class="relative" wire:ignore.self
                        x-data="{ open: false, selected: @js($companyId ?: ''), companies: @js($companies->pluck('name', 'id')), get selectedName() { return this.selected && this.companies[this.selected] ? this.companies[this.selected] : 'All Companies'; } }"
                        ...
                        <div x-cloak x-show="open" x-transition.origin.top
                            ...
                            <button type="button" class="admin-menu-item" :class="{ 'is-active': selected === '' }"
                                @click.stop="selected = ''; open = false; $wire.set('companyId', '')">All
                                Companies</button>
                    ...
                    <table class="w-full text-left border-separate border-spacing-0">
                        <thead>
                            <tr class="bg-[#2ab4c0] border-b-2 border-gray-200">
                                <th
                                    class="px-6 py-2.5 text-[11px] font-bold text-white uppercase tracking-wide rounded-ss-lg">
                                    API Endpoint</th>
                                <th class="px-6 py-2.5 text-[11px] font-bold text-white uppercase tracking-wide">API
                                    Link</th>
                                <th class="px-6 py-2.5 text-[11px] font-bold text-white uppercase tracking-wide">
                                    Description</th>
                                <th
                                    class="px-6 py-2.5 text-[11px] font-bold text-white uppercase tracking-wide rounded-se-lg text-right">
                                    Verification</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="group hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col">
                                            <span
                                                class="text-[11px] font-bold text-gray-900 uppercase">{{ $endpoint->endpoint_name }}</span>
                                            <span
                                                class="text-[10px] text-[#2ab4c0] font-bold uppercase tracking-tight">{{ $endpoint->company->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
                                            class="text-[11px] font-medium text-gray-600">{{ $endpoint->endpoint_link }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-10 text-center text-[11px] text-gray-500">No endpoints
                                        found.</td>
                                </tr>
                            @endforelse
                        </tbody>
